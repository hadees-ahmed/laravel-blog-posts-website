...
@csrf
    ...
    <textarea
        name="body"
        class="border border-gray-400 p-2 w-full">{{old('body',
        isset($post)
        ? $post->body
        : 'fake data fake data fake data fake datafake data fake datafake data fake datafake data fake datafake data fake data')}}
    </textarea>
    @error('body')
    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
    @enderror
    ...
    <textarea
        name="excerpt"
        class="border border-gray-400 p-2 w-full">{{old('excerpt',
        isset($post)
        ? $post->excerpt
        : 'fake data fake datafake data fake datafake data fake datafake data')}}
    </textarea>
    @error('excerpt')
    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
    @enderror
    ...
    <input class="border border-gray-400 p-2 w-full" type="file" name="thumbnail" >
    @error('thumbnail')
    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
    @enderror
